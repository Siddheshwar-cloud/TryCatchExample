...
<div align="center">
  ...
  <a href="https://hardhat.org/hardhat-network/docs"><img src="https://img.shields.io/badge/Network-Local-00FF87?style=for-the-badge" alt="Network"></a>
  <a href="https://ethereum.org/"><img src="https://img.shields.io/badge/Ethereum-Blockchain-627EEA?style=for-the-badge&logo=ethereum&logoColor=white" alt="Ethereum"></a>
  ...
  <img src="https://img.shields.io/github/last-commit/Siddheshwar-cloud/TryCatchExample?style=flat-square&color=00D9FF" alt="Last Commit">
  <img src="https://img.shields.io/github/languages/code-size/Siddheshwar-cloud/TryCatchExample?style=flat-square&color=00FF87" alt="Code Size">
  <img src="https://img.shields.io/github/languages/top/Siddheshwar-cloud/TryCatchExample?style=flat-square&color=FFF04D" alt="Top Language">
...
║           🎉 CONGRATULATIONS! 🎉                             ║
║                                                              ║
║     You've Just Mastered Try-Catch in Solidity!             ║
║                                                              ║
║  ✅ Exception Handling                                       ║
║  ✅ Error Recovery                                           ║
║  ✅ Graceful Failures                                        ║
...
<table align="center">
  <tr>
    <td align="center">
      <img width="60" src="https://img.icons8.com/fluency/96/000000/code.png"/>
      <br><strong>Try-Catch Syntax</strong>
      <br><sub>Master error handling</sub>
    </td>
    <td align="center">
      <img width="60" src="https://img.icons8.com/fluency/96/000000/error.png"/>
      <br><strong>Error Types</strong>
      <br><sub>catch Error(), catch Panic()</sub>
    </td>
    <td align="center">
      <img width="60" src="https://img.icons8.com/fluency/96/000000/security-checked.png"/>
      <br><strong>Graceful Failures</strong>
      <br><sub>Never crash, always recover</sub>
    </td>
    <td align="center">
      <img width="60" src="https://img.icons8.com/fluency/96/000000/rocket.png"/>
...
```mermaid
graph TB
    A[❌ WITHOUT Try-Catch] --> B[External Call Fails]
    B --> C[ENTIRE Transaction Reverts]
    C --> D[😱 User Experience: TERRIBLE]
    C --> E[💸 Gas Wasted]
    C --> F[🔒 Funds Stuck]
    
    G[✅ WITH Try-Catch] --> H[External Call Fails]
    H --> I[Catch Error Gracefully]
...
2. 🛡️ **Production Resilience**: Enterprise code must never crash unexpectedly
...
```mermaid
sequenceDiagram
    participant Main as Your Contract
    participant Ext as External Contract
    participant User as User
    
    User->>Main: Call safeDivide(10, 0)
    
    rect rgb(76, 175, 80)
    Note over Main: Try Block
    Main->>Ext: calculator.divide(10, 0)
    end
    
    rect rgb(244, 67, 54)
    Note over Ext: Error!
    Ext-->>Main: Revert: "Division by zero"
    end
    
    rect rgb(33, 150, 243)
    Note over Main: Catch Block
    Main->>Main: Catch Error(reason)
    Main->>Main: Return ("Division by zero", 0)
    end
    
    Main-->>User: ("Division by zero", 0)
    
    Note over User: ✅ Transaction Succeeds!
    Note over User: Error handled gracefully
```
...
try externalContract.someFunction() returns (ReturnType result) {
    ...
    // ⚠️ Revert with error message (from require)
    ...
    // 🛡️ Catch-all (when you don't need error data)
}
```
...
```mermaid
graph TB
    A[TryCatchExample Contract] --> B[safeDivide Function]
    B --> C[Try Block]
    B --> D[Catch Error Block]
    B --> E[Catch Fallback]
    
    C -->|Success| F[calculator.divide]
    F -->|Returns result| G[Return Success, result]
    
    F -->|require fails| D
    D -->|Has reason| H[Return reason, 0]
    
    F -->|Other error| E
    E -->|Unknown| I[Return Unknown error, 0]
    
    J[Calculator Contract] --> K[divide Function]
    K --> L[require b != 0]
    L -->|Pass| M[Return a / b]
    L -->|Fail| N[Revert with message]
...
async function visualizeErrors() {
  ...
  for (const scenario of scenarios) {
    ...
    if (result[0] === "Success") {
      ...
      console.log("⚠️  Status: ERROR CAUGHT");
      console.log(`📋 Message: ${result[0]}`);
      console.log(`🔢 Fallback Value: ${result[1]}`);
      console.log("🛡️  Execution: Recovered gracefully");
    }
  }
...
│  🛡️  Handle External Call Failures                       │
...
|:----------:|:-------:|:-------:|
| ![Solidity](https://img.shields.io/badge/Solidity-^0.8.20-363636?style=for-the-badge&logo=solidity) | ^0.8.20 | Smart Contracts |
| ![Hardhat](https://img.shields.io/badge/Hardhat-Latest-FFF04D?style=for-the-badge) | Latest | Development |
| ![Ethers](https://img.shields.io/badge/Ethers.js-v6-00D9FF?style=for-the-badge) | v6 | Blockchain Interaction |
| ![Node.js](https://img.shields.io/badge/Node.js-v16+-339933?style=for-the-badge&logo=node.js) | v16+ | Runtime |
...
<div align="center">
  ...
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=Sidheshwar%20Yengudle&fontSize=42&fontColor=fff&animation=twinkling&fontAlignY=32"/>
  
  <br>
  
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=28&pause=1000&color=00D9FF&center=true&vCenter=true&width=600&lines=Blockchain+Developer+%F0%9F%9A%80;Smart+Contract+Architect+%F0%9F%8F%97%EF%B8%8F;Exception+Handling+Expert+%F0%9F%9B%A1%EF%B8%8F;Try-Catch+Master+%E2%9A%A1" alt="Typing SVG" />
